...
- `vendor_panel/` - Next.js vendor/seller frontend
- `visuals/` - Shared TypeScript visual assets package
...
├── vendor_panel/     # Vendor/seller frontend Git submodule
...
| `vendor_panel/` | `git@github.com:Alireza-Rajabzadeh/uz_shop_next.git` |
...
git -C vendor_panel status
git -C visuals status
git status
```
...
- Vendor frontend changes: make them inside `vendor_panel/` and follow `vendor_panel/AGENTS.md`.
- Shared visual package changes: make them inside `visuals/` and publish them before updating frontend lockfiles.
